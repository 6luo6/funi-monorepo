/**
 * 获取执行该方法时所在的目录
 * @returns
 */
declare const useGetCurrentPath: () => string;
/**
 * 判断当前路径是否是文件
 * @param path
 * @returns
 */
declare const useIsFile: (path: string) => boolean;
/**
 * 判断当前路径是否是目录
 * @param path
 * @returns
 */
declare const useIsDirectory: (path: string) => boolean;
/**
 * 如果路径Path的是一个文件则会返回文件名和后罪名，否则返回目录名
 * @param path
 */
declare const useGetFileName: (path: string) => string | {
    name: string;
    suffix: string;
};
/**
 * 获取路径的后缀名
 * @param path
 * @returns
 */
declare const useGetExtensionName: (path: string) => string;
interface ExecCommandOptions {
    cwd: string;
}
/**
 * 执行命令
 * @param command
 * @param options
 */
declare const useExecCommand: (command: string, options?: ExecCommandOptions) => void;
/**
 * 递归删除目录
 * @param path
 */
declare const useRecursionDelete: (path: string) => void;

export { useExecCommand, useGetCurrentPath, useGetExtensionName, useGetFileName, useIsDirectory, useIsFile, useRecursionDelete };
